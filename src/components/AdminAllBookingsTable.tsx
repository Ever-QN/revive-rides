...
export default function AdminAllBookingsTable({ user }: any) {
  ...
  function formatTime(timeString: string): string {
    ...
    return (
      <>
      {loading ? (
        <p className="flex justify-center font-bold">Loading All Appointments...</p>
      ) : (  
      bookings.length === 0 ? (
        <div className="mt-4 flex flex-1 items-center justify-center rounded-lg border border-dashed shadow-sm h-screen">
          <div className="flex flex-col items-center gap-1 text-center">
            ...
            <p className="text-sm text-muted-foreground">
                No appointments have been booked with S&D Autobody yet.
            </p>
          </div>
        </div>
      ) : (
        ...
        <div className="flex flex-col gap-2">
            ...
            <CardDescription>
                View S&amp;D Autobody&apos;s appointment history (including upcoming + cancelled bookings).
            </CardDescription>
        </div>
